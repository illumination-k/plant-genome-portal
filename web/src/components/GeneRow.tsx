...
const GeneRow = (props: { gene: Gene }): ReactElement => (
  <tr>
    ...
    <td className="px-4 py-3 text-zinc-600">{props.gene.assembly_accession}</td>
    <td className="px-4 py-3 text-zinc-600">
      {geneFormat.formatLocation(props.gene.sequence_name, props.gene.region)}
    </td>
    <td className="px-4 py-3 text-zinc-600">{geneFormat.formatStrand(props.gene.strand)}</td>
    <td className="px-4 py-3 text-zinc-600">{props.gene.feature_type}</td>
  </tr>
);
